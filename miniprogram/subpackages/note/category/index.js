...
function decoratePoints(points) {
  return (points || []).map((point) => ({
    ...
    _hasPreviewNodes: Array.isArray(point.previewNodes) && point.previewNodes.length > 0,
    _hasFormulaPreview: /[=+\-×÷*≈±∓^_\/()（）]/.test(point.preview || ""),
  }));
}
...
Page({
  ...
  handlePointTap(event) {
    ...
    wx.navigateTo({
      url: `/subpackages/note/detail/index?moduleId=${this.data.module.id}&pointId=${pointId}`,
    });
  },

  onShareAppMessage() {
    const title = this.data.category ? `${this.data.category.title} - 花生笔记` : '花生笔记';
    return { title };
  },
  onShareTimeline() {
    const title = this.data.category ? `${this.data.category.title} - 花生笔记` : '花生笔记';
